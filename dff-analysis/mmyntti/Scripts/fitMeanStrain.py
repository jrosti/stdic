...
# Creates data and pdf file for the averge strain curve calculated from .dff files.
...
class AveStrain:
	...
	def __init__( self, filenames ):
		...
		for fn in filenames:
			...
			print "Average strain curve for " + fn
			...
			# derivaatta). Some of the y axis gets a bit shorter than the original image was.
			...
			# Another kind of average strain: first average of the movements, then strain.
			avDiff = np.mean(diffY[x1:x2,:],0)
			vCutSY2 = ( avDiff[self.lag:] - avDiff[:-self.lag] ) / (self.lag*self.stepy)

			vCutSY1 = np.mean(strainY[x1:x2,:],0)	# Average strain
			...
			strainData = zip(mmDepth, vCutSY1)
			x = mmDepth # Too lazy to change, I just place it on x.
			x.reverse() # Also reversing for making it to count from the fatigued side
				    # instead from non-fatigued one.
			y1 = vCutSY1  # -||- without reversing
			y2 = vCutSY2

			# TURNING y:s FROM NON-UNITS TO PERCENTS
			...
			out = np.array(zip(mmDepth, vCutSY1), dtype=[('Depth', float),('float', float)])
			outFile = fn.split(".dff")[0] + ".dat"
			...
			for i in range(startIndex, endIndex):
				xf[i-startIndex] = x[i]
				y1f[i-startIndex] = y1[i]
				y2f[i-startIndex] = y2[i]
			fixedData1 = zip(xf, y1f)
			fixedData2 = zip(xf, y2f)

			bounds, coeff, residual = self.polylin(fixedData1, NUM_LIN_FITS) # No numbers, since they are more important.
			bounds2, coeff2, residual2 = self.polylin(fixedData2, NUM_LIN_FITS)	

			# Saving data to text file to text files which can be found from the 
			...
			mpl.plot(x,y1,label="Curve1")
			mpl.plot(x,y2,'r--',label="Curve2")
			...
			for i in np.arange( 0, len(coeff) ):
				...
				mpl.plot([xs[0],xs[0]],[min(y1),max(y1)], 'k:')	# Putting vertical dashed lines between the areas.
			mpl.plot([xs[1],xs[1]],[min(y1),max(y1)], 'k:')		# Putting the last dashed line.
			...
			mpl.ylabel(r'Avg. local strain $\epsilon_y$ [%]', {'color' : 'k', 'fontsize' : 15 })

			# And axis limits
			mpl.ylim(np.min(y1),np.max(y1))
			mpl.xlim(np.min(x),np.max(x))
		
			# Save the single-plots
			fname = fn.split('.dff')[0] + "-avgstrain.pdf"
...
if __name__=="__main__":
    usage = "Usage :\t python display_average_strain_1.0.py filenames"
    ...
    runAnalysis = AveStrain(filenames)
